...
const create = async (entityName, { data }) => {
    const result = await axios.post(plural(entityName), getFormData(data));
    return result.data
};

const update = async (entityName, { query, data }) => {
    const result = await axios.post(`${plural(entityName)}/${query}`, getFormData(data));
    return result.data
}

const remove = async (entityName, { query, isMultiple }) => {
    const result = await isMultiple
        ? axios.post(`${plural(entityName)}/deletes`, JSON.stringify(query))
